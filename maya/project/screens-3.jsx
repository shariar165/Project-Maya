  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: 'radial-gradient(circle, rgba(244,180,200,0.18), transparent 65%)',
        animation: 'splashHalo 2.6s ease-in-out infinite',
      }}/>
      ...
      <style>{`
        ...
        @keyframes splashHalo { 0%,100% { transform: scale(1); opacity: 0.6 } 50% { transform: scale(1.1); opacity: 1 } }
...
// PROFILE
// ──────────────────────────────────────────────────────────────────
function ProfileScreen({ state, setState, openScreen, tweak, setTweak }) {
  const { primaryBtn, ghostBtn, iconBtn } = window.uiBtns;

  const dueDate = (() => {
    // approx due date from week
    const today = new Date(2026, 4, 14);
    const daysRemaining = (40 - state.week) * 7;
    ...
    <div className="screen profile">
      ...
      <div style={{ padding: '14px 22px 0' }}>
        <Card style={{ background: 'linear-gradient(160deg, #FBD7C6, #F4D7E5)', padding: 22, textAlign: 'center' }}>
          <div style={{
            ...
            {(tweak.mothersName || 'M').slice(0,1).toUpperCase()}
          </div>
        ...
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: '#5A3E5F', marginTop: 2 }}>Based on your last period · 12 Aug 2025</div>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div style={{ padding: '16px 22px 0' }}>
        ...
        <Card style={{ padding: 0 }}>
          {[
            { label: 'Name', value: tweak.mothersName, k: 'mothersName', icon: '👤' },
            { label: 'Pregnancy week', value: `Week ${state.week}`, k: 'week', icon: '🌱' },
            { label: 'Age', value: '27 years', icon: '🎂' },
            { label: 'City', value: 'Dhaka, Bangladesh', icon: '📍' },
            { label: 'Blood group', value: 'O+', icon: '🩸' },
            { label: 'First pregnancy', value: 'Yes', icon: '✨' },
            { label: 'Language preference', value: tweak.lang === 'bn' ? 'বাংলা' : tweak.lang === 'en' ? 'English' : 'Mixed', icon: '🌐' },
          ].map((f, i, arr) => (
...
// SETTINGS
// ──────────────────────────────────────────────────────────────────
function SettingsScreen({ state, setState, openScreen, tweak, setTweak }) {
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 10, height: 10, borderRadius: 99, background: L.dot, boxShadow: `0 0 8px ${L.dot}` }}/>
